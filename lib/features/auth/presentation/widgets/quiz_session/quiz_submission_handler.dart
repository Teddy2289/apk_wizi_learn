...
class QuizSubmissionHandler {
  ...
  Future<Map<String, dynamic>> submitQuiz({
    ...
    try {
      ...
      debugPrint('Full quiz response: $response');
  ...
  Map<String, dynamic> _transformResponse(Map<String, dynamic> response) {
    if (response.containsKey('questions') && response['questions'] is List) {
      ...
      final transformedQuestions = questions.map((question) {
          ...
          if (question['selectedAnswers'] is String) {
            return {
              ...
              'selectedAnswers': {
                ...
                'id': _findAnswerId(question['answers'], question['selectedAnswers'])
              }
            };
          }

          // Si c'est une liste, prendre le premier élément
          if (question['selectedAnswers'] is List && question['selectedAnswers'].isNotEmpty) {
            final first = question['selectedAnswers'].first;
            return {
              ...question,
              'selectedAnswers': {
                'text': first is Map ? first['text'] : first.toString(),
                'id': first is Map ? first['id']?.toString() : _findAnswerId(question['answers'], first)
              }
            };
          }
        }
  ...
  static String? _findAnswerId(List<dynamic> answers, dynamic answerValue) {
    try {
      ...
      final answerText = answerValue is Map ? answerValue['text'] : answerValue.toString();
